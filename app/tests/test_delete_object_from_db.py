...
def test_delete_vending_machine() :
    unwanted_vending_machine = setup_db_for_delete(Vending_machine, Vending_machine("testname","testlocation"))
    assert unwanted_vending_machine is not None
    deleteObjFromDB(unwanted_vending_machine)
    session = Session()
    unwanted_vending_machine_after_delete = session.query(Vending_machine).filter_by(id=unwanted_vending_machine.id).first()
...
def test_delete_product() :
    ...
    session = Session()
    unwanted_product_after_delete = session.query(Vending_machine).filter_by(id=unwanted_product.id).first()
...
def test_delete_stock() :
    ...
    session = Session()
    unwanted_stock_after_delete = session.query(Vending_machine).filter_by(id=unwanted_stock.id).first()
